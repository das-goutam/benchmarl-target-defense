...
def solve_apollonius_optimization(attacker_pos: np.ndarray, 
    ...
    try:
        ...
        for defender_pos in defender_positions:
            # Check if defender is at same position as attacker
            distance = np.linalg.norm(defender_pos - attacker_pos)
            if distance < 1e-6:
                # Defender already at attacker position - immediate capture
                return {
                    'success': True,
